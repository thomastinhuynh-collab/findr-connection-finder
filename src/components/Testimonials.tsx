...
const Testimonials = () => {
  return (
    <section className="py-24 relative grain-texture" style={{ backgroundColor: 'hsl(30 20% 94%)' }}>
      <div className="container mx-auto px-4">
        ...
        >
          {guarantees.map((g, i) => (
            <div key={i} className="flex items-center gap-3 px-6 py-3 rounded-full border" style={{ borderColor: 'hsl(0 0% 10% / 0.1)', backgroundColor: 'hsl(38 33% 95%)' }}>
              <g.icon className="w-5 h-5" style={{ color: 'hsl(18 66% 47%)' }} />
              <span className="text-sm font-semibold" style={{ color: 'hsl(0 0% 10%)' }}>{g.label}</span>
            </div>
          ))}
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto mb-16">
          {testimonials.map((t, index) => (
            <motion.div
              ...
              style={{ backgroundColor: 'hsl(38 33% 95%)', borderColor: 'hsl(0 0% 10% / 0.06)' }}
            >
              <div className="flex gap-1 mb-4">
                {Array.from({ length: t.rating }).map((_, i) => (
                  <Star key={i} className="w-4 h-4" style={{ color: 'hsl(36 62% 58%)', fill: 'hsl(36 62% 58%)' }} />
                ))}
              </div>

              <p className="text-sm leading-relaxed mb-6" style={{ color: 'hsl(0 0% 10% / 0.75)' }}>
                "{t.quote}"
              </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: 'hsl(0 0% 10%)' }}>{t.author}</p>
                  <p className="text-xs" style={{ color: 'hsl(0 0% 10% / 0.45)' }}>{t.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <motion.div
          ...
          style={{ backgroundColor: 'hsl(0 0% 10%)', borderColor: 'hsl(0 0% 18%)' }}
        >
          <div className="flex items-center justify-center gap-2 mb-3">
            <Users className="w-5 h-5" style={{ color: 'hsl(18 66% 55%)' }} />
            <span className="text-sm font-barlow font-medium" style={{ color: 'hsl(38 33% 93% / 0.6)' }}>Liste d'attente</span>
          </div>

          <p className="text-2xl md:text-3xl font-serif font-bold mb-1" style={{ color: 'hsl(38 33% 93%)' }}>
            Déjà {WAITLIST_COUNT.toLocaleString('fr-FR')} curieux inscrits
          </p>
          <p className="text-xs mb-5" style={{ color: 'hsl(38 33% 93% / 0.4)' }}>
            Objectif : {WAITLIST_GOAL.toLocaleString('fr-FR')} inscrits
          </p>

          <div className="h-3 rounded-full overflow-hidden" style={{ backgroundColor: 'hsl(38 33% 93% / 0.1)' }}>
            <motion.div
              initial={{ width: 0 }}
              ...
              style={{ background: 'linear-gradient(90deg, hsl(18 66% 47%), hsl(36 62% 58%))' }}
            />
          </div>
        </motion.div>
      </div>
